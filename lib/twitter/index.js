                ...
                req({
                    ...
                    if (error) {
                        ...
                        try {
                            ...
                            if (!info['globalObjects']['tweets'][id]['extended_entities']) return callback(new Error('No media'));
                            const media = info['globalObjects']['tweets'][id]['extended_entities']['media']
                            if (media[0].type == 'video') callback(null, {
                                    type: media[0].type,
                                    full_text: info['globalObjects']['tweets'][id]['full_text'],
                                    variants: media[0]['video_info']['variants']
                                });
                            if (media[0].type == 'photo') callback(null, {
                                    type: media[0].type,
                                    full_text: info['globalObjects']['tweets'][id]['full_text'],
